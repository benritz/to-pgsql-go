...
func main() {
	...
	flag.BoolVar(&forceCaseInsensitive, "forceCaseInsensitive", true, "Enable citext for case-insensitive text columns")
...
func writeTables(db *sql.DB, out *os.File) error {
	out.WriteString("/* --------------------- TABLES --------------------- */\n\n")
	if forceCaseInsensitive {
		out.WriteString("-- enable case insensitive extension\n")
		out.WriteString("create extension citext;\n\n")
	}
	...
	for rows.Next() {
		...
		columns = append(columns, Column{ColumnID: columnID, Name: columnName, MaxLength: maxLength, Precision: precision, Scale: scale, IsNullable: isNullable, IsIdentity: isIdentity, Type: colType})
	}
	if lastTable != "" {
		tables = append(tables, Table{Name: lastTable, Columns: columns})
	}
...
func writeTable(db *sql.DB, out *os.File, table string, columns []Column) error {
	columnDefs := ""
	...
	for rows.Next() {
		...
		if dataBatchSize == 1 {
			out.WriteString(fmt.Sprintf("insert into %s values (%s);\n", table, strings.Join(valStrs, ", ")))
		} else {
			if n%dataBatchSize == 0 {
				if n != 0 {
					out.WriteString(";\n\n")
				}
				out.WriteString(fmt.Sprintf("insert into %s values (%s)", table, strings.Join(valStrs, ", ")))
			} else {
...
func writeSeqReset(out *os.File) {
	out.WriteString("-- function to find a sequence's field's maximum value, this is used to set the sequence's next value after the data is inserted\n")
	out.WriteString("-- see http://stackoverflow.com/a/5943183/1095458\n")
	out.WriteString(`create or replace function seq_field_max_value(oid) returns bigint
...
begin
 for tabrelid, colname in select attrelid, attname
  ...
  return newmax;
	...
	out.WriteString("-- set any sequence to the maximum value of the sequence's field\n")
	out.WriteString("select relname, setval(oid, seq_field_max_value(oid)) from pg_class where relkind = 'S';\n\n")
}

func writeIndexes(db *sql.DB, out *os.File) error {
	out.WriteString("/* --------------------- PRIMARY KEYS, UNIQUE CONSTRAINTS AND INDEXES --------------------- */\n\n")
	...
	for rows.Next() {
		...
		if indexColumnID == 1 {
			if !first {
				if isPrimary {
					out.WriteString(fmt.Sprintf("alter table %s add constraint %s primary key (%s);\n", lastTable, lastIndex, strings.Join(columns, ", ")))
				} else if isUniqueConst {
					out.WriteString(fmt.Sprintf("alter table %s add constraint %s unique (%s);\n", lastTable, lastIndex, strings.Join(columns, ", ")))
				} else if isUnique {
					out.WriteString(fmt.Sprintf("create unique index %s on %s (%s);\n", lastIndex, lastTable, strings.Join(columns, ", ")))
				} else {
					out.WriteString(fmt.Sprintf("create index %s on %s (%s);\n", lastIndex, lastTable, strings.Join(columns, ", ")))
				}
...
func writeForeignKeys(db *sql.DB, out *os.File) error {
...
func writeFunctions(db *sql.DB, out *os.File) error {
	out.WriteString("/* --------------------- FUNCTIONS --------------------- */\n\n")
	out.WriteString("/* -- These functions contain T-SQL and MUST be rewritten for PGSQL -- */\n\n")
	return writeCompiledObject(db, out, "FN")
}

func writeDefaults(db *sql.DB, out *os.File) error {
	...
	for rows.Next() {
		...
		out.WriteString(fmt.Sprintf("alter table %s alter column %s set default %s;\n", tableName, columnName, def))
	}
	out.WriteString("\n")
	return nil
}

func writeProcedures(db *sql.DB, out *os.File) error {
	out.WriteString("/* --------------------- PROCEDURES --------------------- */\n\n")
	out.WriteString("/* -- These procedures contain T-SQL and MUST be rewritten for PGSQL -- */\n\n")
	return writeCompiledObject(db, out, "P")
}

func writeViews(db *sql.DB, out *os.File) error {
	out.WriteString("/* --------------------- VIEWS --------------------- */\n\n")
	out.WriteString("/* -- These views may contain T-SQL and may need to be rewritten for PGSQL -- */\n\n")
	return writeCompiledObject(db, out, "V")
}

func writeTriggers(db *sql.DB, out *os.File) error {
	out.WriteString("/* --------------------- TRIGGERS --------------------- */\n\n")
	out.WriteString("/* -- These triggers contain T-SQL and MUST be rewritten for PGSQL -- */\n\n")
	return writeCompiledObject(db, out, "TR")
}

// Helper for compiled objects (functions, procedures, views, triggers)
func writeCompiledObject(db *sql.DB, out *os.File, objType string) error {
